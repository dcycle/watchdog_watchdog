...
echo ''
echo '-----'
echo 'About to create the watchdog_watchdog_default network if it does not exist,'
...
docker network ls | grep watchdog_watchdog_default || docker network create watchdog_watchdog_default
