...
echo " => Installing watchdog_watchdog_severity"
docker-compose exec -T drupal /bin/bash -c 'drush en -y watchdog_watchdog_severity'
...
docker-compose exec -T drupal /bin/bash -c 'drush pmu -y watchdog_watchdog_severity'
